 ...
 * @author jlmd
 */
public class LeftArcView extends View implements ComponentAnimation {
  ...
  public LeftArcView(Context context) {
    super(context);
    init();
  }

  public LeftArcView(Context context, AttributeSet attrs) {
    super(context, attrs);
    init();
  }

  public LeftArcView(Context context, AttributeSet attrs, int defStyleAttr) {
  ...
  private void drawArc(Canvas canvas) {
    canvas.drawArc(oval, 100, drawAngle, false, paint);
  }

  @Override
  public void startAnimation(final Callback callback) {
    setVisible();
    ObjectAnimator rotation = ObjectAnimator.ofFloat(this, "rotation", 0f, 163);
  ...
  private void startResizeDownAnimation() {
    ValueAnimator valueAnimator = ValueAnimator.ofInt(MAX_ANGLE,
        MIN_ANGLE);
